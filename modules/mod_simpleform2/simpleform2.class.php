		...
		switch($elem->type){
			...
			case 'radio':
				foreach($elem->options as $option){
					...
					$optionCode = '<input type="radio" name="'.$name.'" id="'.$id.'" value="'.$option->value.'"'.($class?' class="'.$class.'"':'').($onclick?' onclick="'.$onclick.'"':'').$sel.' />'.$option->label.'<br>';
					$result = str_replace($option->code,$optionCode,$result);
				}
			break;
			case 'button':
			...
			case 'captcha':
				...
				if((int)$user->get('id')>0){
					...
					$result.= '<img class="sf2Captcha" src="'.JURI::root().'modules/mod_simpleform2/engine.php?task=captcha'.(count($urlAdd)?'&'.implode('&',$urlAdd):'').'" alt="'.JText::_('Click to refresh').'" title="'.JText::_('Click to refresh').'" onclick="'.$onclick.'"'.' style="cursor:pointer;" />
