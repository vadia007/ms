	...
	function loadAction($form, $actiondata){
		...
		if($form->form_params->get('add_form_tags', 1)){
			...
			if(trim($form->form_params->get('action_url', ''))){
				...
				if($form->form_params->get('relative_url', 1) == 1){
					...
					if((bool)$form->disguised === true){
						...
						if(isset($form->data['task']) && !empty($form->data['task']) && strpos($action_url, $form->data['task']) !== false){
							...
							$action_url = str_replace($form->data['task'], $form->form_params->get('action_task', $params->get('submit_event', 'submit')), $action_url);
						}else{
							$action_url .= $separator.'task='.$form->form_params->get('action_task', $params->get('submit_event', 'submit'));
						}
					}else{
				...
				//check if we are submitting to _self
				if($params->get('submit_event', 'submit') == '_self'){
					preg_match_all('/((&|&amp;|\?)*)(event|task)=([^&]+)/is', $action_url, $event_matches);
					$action_url = str_replace($event_matches[0], '', $action_url);
				}
			...
			//add the form class
			$form_tag_array[] = 'class="Chronoform'.$form->form_params->get('html_form_class', '').'"';
			//add any attachments
		...
		if((int)$params->get('curly_replacer', 1) == 1){
			$output = $form->curly_replacer($output, $form->data, '.', true);
		}
		...
		if(strlen(trim($params->get('form_container', ''))) > 0){
			$output = '<div id="'.trim($params->get('form_container', '')).'">'.$output.'</div>';
		}
		echo $output;
	}
    ...
    function _loadValidationScripts($form){
		if($form->loaded_validation === false){
			...
			window.addEvent('domready', function() {
				document.id('<?php echo $form->html_form_id; ?>').addClass('hasValidation');
