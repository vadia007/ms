...
<div class="element_code" id="cfaction_upload_files_element">
	...
	<input type="hidden" name="chronoaction[{n}][action_upload_files_{n}_zip_file]" id="action_upload_files_{n}_zip_file" value="<?php echo $action_params['zip_file']; ?>" />
	<input type="hidden" name="chronoaction[{n}][action_upload_files_{n}_zip_delete_file]" id="action_upload_files_{n}_zip_delete_file" value="<?php echo $action_params['zip_delete_file']; ?>" />
...
<div class="element_config" id="cfaction_upload_files_element_config">
	...
	<?php echo $PluginTabsHelper->tabStart('advanced'); ?>
		...
		<?php echo $HtmlHelper->input('action_upload_files_{n}_zip_file_config', array('type' => 'select', 'label' => 'Compress file', 'options' => array(0 => 'No', 1 => 'Yes'), 'smalldesc' => 'Would you like to compress the uploaded file ? only .zip compression is supported and this option requires PHP 5.2 or later')); ?>
		<?php echo $HtmlHelper->input('action_upload_files_{n}_zip_delete_file_config', array('type' => 'select', 'label' => 'Delete original', 'options' => array(0 => 'No', 1 => 'Yes'), 'smalldesc' => 'Delete original file after compression ?')); ?>
